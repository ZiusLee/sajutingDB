...
export function handleRateLimitError(error: any) {
  if (error.status === 429) {
    ...
    const resetTime = resetAt ? resetAt.toLocaleTimeString() : "알 수 없음"

    return {
      error: true,
      message: `API 호출 제한을 초과했습니다. ${resetAt ? `${resetTime}에 다시 시도해주세요.` : "잠시 후 다시 시도해주세요."}`,
      resetAt,
    }
  }
...
export async function getInterpretation(
  ...
  try {
    const response = await fetch("/api/saju-interpretation", {
      ...
      body: JSON.stringify({ saju, name, gender, model, relationshipStatus }),
    })
