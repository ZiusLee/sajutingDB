...
interface SignupDialogProps {
  ...
  onSelectProvider: (provider: "kakao" | "google" | "apple") => void
}

export function SignupDialog({ open, onOpenChange, onSelectProvider }: SignupDialogProps) {
  const [showTerms, setShowTerms] = useState(false)
  const [showPrivacyDetails, setShowPrivacyDetails] = useState(false)
  const [selectedProvider, setSelectedProvider] = useState<"kakao" | "google" | "apple" | null>(null)
  ...
  const handleProviderSelect = (provider: "kakao" | "google" | "apple") => {
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col space-y-2">
              <Button onClick={handleTermsAgree} disabled={!agreedToTerms || !agreedToPrivacy} className="w-full">
                ...
                {selectedProvider === "apple" && "Apple로 시작하기"}
              </Button>
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange} modal={true}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-center">사주핑 시작하기</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <p className="text-center text-sm text-muted-foreground">
            간편하게 로그인하고 나만의 사주 분석을 시작해보세요
          </p>
          ...
          <div className="space-y-3">
            ...
            >
              <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 3C6.48 3 2 6.48 2 10.8c0 2.76 1.8 5.16 4.44 6.6L5.4 20.4c-.12.36.24.72.6.48l3.6-2.4c.84.12 1.68.12 2.4.12 5.52 0 10-3.48 10-7.8S17.52 3 12 3z" />
              </svg>
              카카오로 시작하기
            </Button>

            <Button onClick={() => handleProviderSelect("google")} variant="outline" className="w-full">
          ...
          <div className="text-center">
            <p className="text-xs text-muted-foreground">
              계속 진행하면 <button className="text-blue-600 hover:underline">서비스 이용약관</button> 및{" "}
              <button className="text-blue-600 hover:underline">개인정보 처리방침</button>에 동의하는 것으로 간주됩니다.
            </p>
          </div>
        </div>
      </DialogContent>
