...
export function CoinPurchaseModal({ isOpen, onClose, onSuccess }: CoinPurchaseModalProps) {
  ...
  const handlePurchase = async (packageId: string) => {
    ...
    // ===== 결제 기능 임시 비활성화 =====
    // TODO: 결제 기능 활성화 시 아래 주석을 해제하고 이 부분을 제거
    alert("결제 기능 준비 중입니다. 곧 서비스 예정입니다!")
    return
    // ===== 결제 기능 임시 비활성화 끝 =====
    ...
    try {
      // 클라이언트 키 가져오기
      const clientKeyResponse = await fetch("/api/payments/client-key")
      if (!clientKeyResponse.ok) {
        throw new Error("클라이언트 키를 가져올 수 없습니다.")
      }
      const { clientKey } = await clientKeyResponse.json()
