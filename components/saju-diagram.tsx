...
import SajuChart from "@/components/saju-chart"
...
export default function SajuDiagram({
  ...
  if (variant === "card") {
    return (
      <div className="bg-background rounded-2xl border border-border p-4 shadow-sm">
        <ProfileInfo name={displayName} dayMasterInfo={dayMasterInfo} variant="card" />
        <SajuChart saju={activeSaju} timeUnknown={timeUnknown} variant="sidebar" />
      </div>
    )
  }
    ...
    return (
      <>
        <div className="p-3">
          <Collapsible open={!profileCollapsed} onOpenChange={(open) => setProfileCollapsed(!open)}>
            ...
            <CollapsibleContent className="space-y-3 mt-2">
              <ProfileInfo
                ...
                birthInfo={{
                  birthYear: defaultProfile?.birthYear || solarYear,
                  birthMonth: defaultProfile?.birthMonth || solarMonth,
                  birthDay: defaultProfile?.birthDay || solarDay,
                  birthHour: defaultProfile?.birthHour || hour,
                  birthMinute: defaultProfile?.birthMinute || minute,
                  timeUnknown: defaultProfile?.timeUnknown ?? timeUnknown,
                  location: defaultProfile?.location || location,
                  gender: defaultProfile?.gender || gender,
                }}
                variant="sidebar"
              />
          ...
          {/* Profile Management Dialog - keeping existing complex logic */}
  ...
  return (
    <div className="space-y-4">
      ...
      <SajuChart saju={activeSaju} timeUnknown={timeUnknown} variant="chat" />
    </div>
  )
}
