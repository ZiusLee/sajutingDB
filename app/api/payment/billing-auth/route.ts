...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[Billing Auth] Processing billing authorization:", {
      authKey: !!authKey,
      customerKey,
      orderId,
      packageId,
    ...
    if (!authKey || !customerKey || !orderId || !packageId) {
      console.error("[Billing Auth] Missing required fields")
    ...
    if (userError || !user) {
      console.error("[Billing Auth] Authentication failed:", userError)
      return NextResponse.json({ error: "Authentication required" }, { status: 401 })
    }

    // Confirm billing auth with Toss Payments
    console.log("[Billing Auth] Confirming billing auth with Toss Payments")
    ...
    if (!tossResponse.ok) {
      ...
      console.error("[Billing Auth] Toss billing auth confirmation failed:", errorData)
      return NextResponse.json(
        {
          error: "Billing authorization failed",
          details: errorData.message || "Unknown Toss API error",
        },
        { status: 400 },
      )
    }
    ...
    console.log("[Billing Auth] Billing key received:", !!billingKey)
    ...
    console.log("[Billing Auth] Billing authorization completed successfully")
    ...
    console.error("[Billing Auth] Billing auth confirmation error:", error)
    return NextResponse.json(
      {
