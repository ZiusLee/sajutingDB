...
// 메시지 처리 및 컨텍스트 최적화 함수 - 유저 질문만 최근 3개까지 포함
async function processMessagesForContext(messages: any[], saju: any, name: string, roomType: string) {
  // 초기 메시지들 (message_order 0, 1) 유지
  const initialMessages = messages.slice(0, 2)

  // 나머지 메시지에서 유저 메시지만 필터링
  const userMessages = messages.slice(2).filter((msg) => msg.role === "user")

  // 최근 유저 질문 3개까지만 선택
  const recentUserMessages = userMessages.slice(-3)

  // 초기 메시지 + 최근 유저 질문들만 반환
  return [...initialMessages, ...recentUserMessages]
}
