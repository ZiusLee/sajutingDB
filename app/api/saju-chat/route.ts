...
function getModelForRoomType(roomType: string): string {
  try {
    switch (roomType) {
      case "sajuping":
        return "gpt-4.1" // 사주핑용 모델
      case "tarot":
        return "gpt-4.1" // 타로핑용 모델
      default:
        return "gpt-4.1"
    }
  } catch (error) {
...
export async function POST(req: Request) {
  try {
    const { messages, saju, name, gender, initialInterpretation, roomType, userId, birthInfo } = await req.json()
...
4. 사용자의 질문이 추상적일 경우, 구체화를 유도하는 보조질문을 1~2개 제시하세요.
...
사용자가 한번 타로카드의 번호를 불러 뽑으면 다시 카드를 랜덤하게 섞어 같은번호를 불러도 같은 타로카로가 나오지 않게 해야합니다.
