...
import { ArrowLeft, Calendar, Clock, Eye, LinkIcon, RefreshCw, Bug, StarIcon, CheckCircle2 } from "lucide-react"
...
export default function EditSajuPage() {
  ...
  const loadUserData = async () => {
    try {
      ...
      if (sessionError || !sessionData?.session) {
        router.push("/login?returnUrl=/edit-saju")
        return
      }
      ...
      if (defaultProfile) {
        setDefaultSessionId(defaultProfile.id)
      }
    } catch (error) {
  ...
  // 메인 사주 설정
  const handleSetDefaultSaju = async (sessionId: string) => {
    try {
      if (!authUserId) {
        toast({
          title: "오류",
      ...
      toast({
        title: "메인 사주 설정 중...",
        description: "선택한 사주를 메인 사주로 설정하고 있습니다.",
      })

      const success = await setDefaultSajuSession(authUserId, sessionId)

      if (success) {
        setDefaultSessionId(sessionId)
        toast({
          title: "메인 사주 설정 완료",
          description: "선택한 사주가 메인 사주로 설정되었습니다.",
        })
        await loadUserData()
      } else {
        throw new Error("메인 사주 설정에 실패했습니다.")
      }
    } catch (error) {
      console.error("Error setting default saju:", error)
      toast({
        title: "메인 사주 설정 오류",
        description: "메인 사주를 설정하는 중 오류가 발생했습니다.",
        variant: "destructive",
      })
    }
  }
              ...
              >
                ...
                <CardContent>
                  <div className="flex flex-col space-y-2">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span>{profile.timeUnknown ? "시간 미상" : `${profile.birthHour}:${profile.birthMinute}`}</span>
                    </div>
                ...
                <CardContent>
                  <div className="flex justify-end gap-2">
                    <Button
                      variant={profile.id === defaultSessionId ? "default" : "outline"}
                      size="sm"
                      onClick={(e) => {
                        e.stopPropagation()
                        handleSetDefaultSaju(profile.id)
                      }}
                      className="gap-1"
                    >
                      {profile.id === defaultSessionId ? (
                        <>
